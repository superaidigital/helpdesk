...
// --- จัดการ Action ทั้งหมดผ่าน POST method ---
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // --- Actions อื่นๆ ที่ต้องมี issue_id ---
    if ($issue_id === 0) {
        redirect_with_message('it_dashboard.php', 'error', 'Invalid Issue ID');
    }

    // --- ดึงข้อมูล issue เพื่อตรวจสอบสิทธิ์ ---
    ...
    if (isset($_POST['submit_update']) && $can_perform_actions) {
        ...
        if (!empty($comment_text)) {
            ...
            if (isset($_FILES['comment_files']) && count(array_filter($_FILES['comment_files']['name'])) > 0) {
                // ... (โค้ดอัปโหลดไฟล์เหมือนเดิม) ...
            }
        }
        
        if ($new_status === 'done') {
            // ... (โค้ดส่งอีเมลแจ้งปิดงานเหมือนเดิม) ...
        }
    ...
    if (isset($_POST['submit_kb']) && $can_perform_actions) {
        // ... (โค้ดเก็บ KB เหมือนเดิม) ...
    } elseif (isset($_POST['submit_kb'])) {
        redirect_with_message("issue_view.php?id=$issue_id", 'error', 'คุณไม่มีสิทธิ์ดำเนินการ หรือเรื่องถูกปิดไปแล้ว');
    }
}

// Fallback redirect หากไม่มี action ตรงกัน
