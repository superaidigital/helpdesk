...
foreach($all_issues_for_month as $issue) {
...
for ($i = 5; $i >= 0; $i--) {
    $current_month_date = new DateTime("$year-$month-01");
    $current_month_date->modify("-$i months");
    $loop_year = $current_month_date->format('Y');
    $loop_month = $current_month_date->format('m');
    $loop_start_date = "$loop_year-$loop_month-01";
    $loop_end_date = date('Y-m-t', strtotime($loop_start_date));
...
$trend_assigned_json = json_encode($trend_data['assigned']);
$trend_completed_json = json_encode($trend_data['completed']);
...
<head>
    ...
    <style>
        ...
        @media print {
            ...
            /* Table header repeat styles */
            #detailed-list-table thead { display: table-header-group; }
            #detailed-list-table tfoot { display: table-footer-group; }
            #detailed-list-table tbody tr { page-break-inside: avoid; }
            #detailed-list-table td, #detailed-list-table th { white-space: nowrap; text-align: center; }
            #detailed-list-table .title-column { white-space: normal; word-break: break-word; text-align: left; }
            #detailed-list-table .reporter-column { text-align: left; }
            #detailed-list-table .shrinkable { width: 1%; }
        }
    </style>
</head>
    ...
    <div class="a4-page">
        ...
        <div class="space-y-4">
             <div class="border p-4 rounded-lg"> 
                <h3 class="text-base font-semibold text-center mb-2">แนวโน้มปริมาณงาน 6 เดือนล่าสุด</h3> 
            ...
            <div class="grid grid-cols-2 gap-6 print-cols-1 print-gap-4">
                ...
                <div class="border p-4 rounded-lg">
                    ...
                    <div class="h-40"><canvas id="urgencyChart"></canvas></div>
                </div>
            </div>
            <div class="grid grid-cols-2 gap-6 print-cols-1 print-gap-4 mt-6">
    ...
    <div class="a4-page page-break">
        <div class="mt-6">
            <h3 class="text-lg font-semibold text-center mb-2">รายการให้บริการแจ้งปัญหาและให้คำปรึกษาด้าน IT ทั้งหมดในเดือนนี้</h3>
             <table id="detailed-list-table" class="min-w-full text-xs border-collapse border border-slate-400">
                <thead class="bg-gray-100">
                    <tr>
                        <th class="border border-slate-300 p-1 shrinkable">ลำดับ</th>
                        <th class="border border-slate-300 p-1 shrinkable">วันที่แจ้ง</th>
                        <th class="border border-slate-300 p-1 title-column">หัวข้อ</th>
                        <th class="border border-slate-300 p-1 reporter-column">ผู้แจ้ง</th>
                        <th class="border border-slate-300 p-1 shrinkable">สถานะ</th>
                        <th class="border border-slate-300 p-1 shrinkable">วันที่เสร็จสิ้น</th>
                        <th class="border border-slate-300 p-1 shrinkable">ใช้เวลาแก้ไข</th>
                    </tr>
                </thead>
                <tbody>
                    <?php if (count($all_issues_for_month) > 0): $counter = 1; foreach($all_issues_for_month as $issue): ?>
                    <tr class="border-b">
                        <td class="border border-slate-300 p-1 shrinkable"><?php echo $counter++; ?></td>
                        <td class="border border-slate-300 p-1 shrinkable"><?php echo date('d/m/', strtotime($issue['created_at'])) . (date('Y', strtotime($issue['created_at'])) + 543); ?></td>
                        <td class="border border-slate-300 p-1 title-column"><?php echo htmlspecialchars($issue['title']); ?></td>
                        <td class="border border-slate-300 p-1 reporter-column"><?php echo htmlspecialchars($issue['reporter_name']); ?></td>
                        <td class="border border-slate-300 p-1 shrinkable"><?php echo $status_text_map[$issue['status']] ?? htmlspecialchars($issue['status']); ?></td>
                        <td class="border border-slate-300 p-1 shrinkable"><?php echo $issue['completed_at'] ? (date('d/m/', strtotime($issue['completed_at'])) . (date('Y', strtotime($issue['completed_at'])) + 543)) : '-'; ?></td>
                        <td class="border border-slate-300 p-1 shrinkable"><?php echo formatDuration($issue['resolution_minutes']); ?></td>
                    </tr>
                    <?php endforeach; else: ?>
